...
def event_list(request):
    ...
    # Using aggregate with Sum to get the total number of unique participants
    total_participants_across_events = Participant.objects.aggregate(
        total=Count('id')
    )['total'] or 0
...
def home(request):
    ...
    # If search query exists, search across all events regardless of filter
    if search_query:
        events = base_queryset.filter(
            Q(name__icontains=search_query) | Q(location__icontains=search_query)
        ).order_by('-date', '-time')
        filter_label = "Search Results"
        filter_param = 'search'  # Set special filter param for search
    # Otherwise, filter events based on the filter parameter
